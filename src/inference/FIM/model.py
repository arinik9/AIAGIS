...
class MyModel(nn.Module):
	def __init__(self, nNodes):
		...
		self.weight=nn.Parameter(torch.FloatTensor(nNodes, nNodes).to(device), requires_grad=True)
		#self.weight=nn.Parameter(torch.zeros(size=(nNodes, nNodes), requires_grad=True, device=device, dtype=torch.float))
