...
def BCELoss(pred_y, true_y, debug=False):
	if debug:
		print(pred_y)
	a = torch.clip(pred_y,1e-10,1.0)
	if debug:
		print(a)
	b = torch.log(a)
	if debug:
	...
	#print(torch.clip((1.0 - pred_y), 1e-10, 1.0))
	#print(torch.clip(pred_y, 1e-10, 1.0))
	#cross_loss=torch.mean(-true_y*torch.log(torch.clip(pred_y,1e-10,1.0))-(1.0-true_y)*torch.log(torch.clip((1.0-pred_y),1e-10,1.0)))
	cross_loss=torch.mean(-true_y*torch.log(torch.clip(pred_y,0.000001,1.0))-(1.0-true_y)*torch.log(torch.clip((1.0-pred_y),0.000001,1.0)))
	return cross_loss
...
def CreateFile(folder, tau, TimeHorizon):
...
def LoadGraph(folder, tau, TimeHorizon, train_per, val_per):
	DataFile=folder+'/cascades-'+str(tau)+'.npy'
	data_exists=exists(DataFile)
	if data_exists==False:
		CreateFile(folder, tau, TimeHorizon)
		print('File Created!')
