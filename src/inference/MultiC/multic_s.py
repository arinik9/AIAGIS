...
def prepare_non_zero_cascade_data(df_events, cascade_list, map_info_data, N):
    ...
    for i in range(C):
        ...
        df_sub["event_id"] = df_sub["id"]
        df_sub.sort_values(by=["timestamp_in_day"], inplace=True)
        #df_sub["timestamp_in_day"] = df_sub["timestamp_in_day"] - 10
        df_sub["timestamp_in_day"] = df_sub["timestamp_in_day"]/7
        data_nz[i] = list(zip(df_sub["timestamp_in_day"], df_sub["newid"], df_sub["event_id"]))  # (infection time, location id, event id)
        #df_sub.sort_values(by=["timestamp_in_hour"], inplace=True)
        #df_sub["timestamp_in_hour"] = df_sub["timestamp_in_hour"]-240
        #data_nz[i] = list(zip(df_sub["timestamp_in_hour"], df_sub["newid"], df_sub["event_id"]))  # (infection time, location id, event id)
    return data_nz
...
def construct_tensors_from_nonzero_cascades_single_layer_phase(data_nz, N, C, T_end, G):
    # G: genome dist matrix
    print("T_end", T_end)
    ...
    time_dict = {c: {e: 0.0 for e in range(N * N)} for c in range(C)}  # transmission edge'lere index atiyo hash yontemiyle, o yuzden init yaparak N*N yapiyo
    genome_dict = {c: {e: 0.0 for e in range(N * N)} for c in range(C)}
    ...
    for c in range(C):
        ...
        for (t, j, e_j) in sim_logs:  # for each cascade c, it returns a list of tuples (t,j) stating that node j is infected at time t.
            if t > T_end:
                break
            ...
            if j in fail_users:  # it ensures that we do not add the same location id twice
                for (u, t_u, e_u) in succ_users:  # the previously infected locations (succ_users) might infect the location j
                    if u != j and t - t_u>0:
                        idx = u * N + j  # index of the directed edge (u, j)
                        # nejat.append((u, j, idx))
                        succ_mask_dict[c].add(idx)
                        succ_edges.add(idx)
                        #time_dict[c][idx] += t - t_u
                        time_dict[c][idx] = t - t_u
                        gval = G.loc[e_j, str(e_u)]
                        if gval != '':
                            genome_dict[c][idx] = (1.0 - (1.0 - float(gval))**2) # distance value transformation with squared power >> 0.5 -> 0.25

                        else:
                            genome_dict[c][idx] = 1.0
                    else:
                        break
                succ_users.insert(0, (j, t, e_j))  # node j is activated at time t on cascade c
                fail_users.remove(j)

        for (j, t_j, e_j) in succ_users:  # for each activated node j on cascade c
            # note that t_j is always <= T_end
            for n in fail_users:
                idx = j * N + n
                fail_mask_dict[c].add(idx)
                fail_edges.add(idx)
                #time_dict[c][idx] += T_end - t_j
                time_dict[c][idx] = T_end - t_j
                genome_dict[c][idx] = 1.0

    succ_edges_list = list(succ_edges)
    fail_edges_list = list(fail_edges)
    return time_dict, genome_dict, succ_mask_dict, fail_mask_dict, succ_edges_list, fail_edges_list
...
def construct_mask_and_scatter_idx_single_layer_phase(N, C, time_dict, genome_dict, succ_mask_dict, succ_edges_list,
    ...
    succ_delta_g_dict = {c: set() for c in range(C)}
    succ_delta_t_dict = {c: set() for c in range(C)}
    fail_delta_t_dict = {c: set() for c in range(C)}
    for c in range(C):
        print("c:", c)
        # Construct time tensor
        succ_time_val = np.array(list(time_dict[c].values()))[succ_edges_list]  # keep only successful activation times
        max_val = np.max(succ_time_val)
        #succ_time_val = (succ_time_val/max_val)**(1/2)
        succ_delta_t = torch.FloatTensor(succ_time_val).to(device)
        succ_delta_t_dict[c] = succ_delta_t
        fail_time_val = np.array(list(time_dict[c].values()))[fail_edges_list]  # keep only successful activation times
        fail_delta_t = torch.FloatTensor(fail_time_val).to(device)
        fail_delta_t_dict[c] = fail_delta_t
        succ_g_val = np.array(list(genome_dict[c].values()))[succ_edges_list]  # keep only successful activation times
        #succ_g_val = max_val*succ_g_val
        succ_delta_g = torch.FloatTensor(succ_g_val).to(device)
        succ_delta_g_dict[c] = succ_delta_g
    ...
    return succ_mask_idx, succ_scatter_idx, succ_delta_g_dict, succ_delta_t_dict, fail_mask_idx, fail_scatter_idx, fail_delta_t_dict, spatial_delta_dist, succ_node_idx
...
def perform_data_parsing_and_tensor_construction_single_layer_phase(df_events, date_start, date_end,
                                                                    cascade_list, D, genome_dist_df, map_info_data, out_dist_values_filepath=None):
    # init
    ...
    time_dict, genome_dict, succ_mask_dict, fail_mask_dict, succ_edges_list, fail_edges_list = construct_tensors_from_nonzero_cascades_single_layer_phase(data_nz, N, C, T_end, genome_dist_df)
    spatial_dist_list_for_succ_edges_init = construct_dist_list_for_succ_edges_single_layer_phase(N, D, succ_edges_list)
    spatial_dist_list_for_succ_edges = spatial_dist_list_for_succ_edges_init
    ...
    succ_mask_idx, succ_scatter_idx, succ_delta_g_dict, succ_delta_t_dict, fail_mask_idx, fail_scatter_idx, fail_delta_t_dict, spatial_delta_dist, succ_node_idx = construct_mask_and_scatter_idx_single_layer_phase(
                                                                            N, C, time_dict, genome_dict, succ_mask_dict, succ_edges_list,
                                                                            fail_mask_dict, fail_edges_list,
                                                                             spatial_dist_list_for_succ_edges)
    ...
    return succ_edges_list, succ_mask_idx, succ_scatter_idx, succ_delta_g_dict, succ_delta_t_dict, fail_edges_list, fail_mask_idx, fail_scatter_idx, fail_delta_t_dict, spatial_delta_dist, succ_node_idx
...
def perform_optimization_single_layer_phase(obj_func, opt_params, N, C,
                                            succ_mask_idx, succ_scatter_idx, succ_node_idx, succ_delta_g_dict, succ_delta_t_dict,
                                            fail_mask_idx, fail_scatter_idx, fail_delta_t_dict,
    ...
    for it in range(max_iter):
        ...
        if obj_func == "dist_survival":
            loss = objective_single_layer_phase_with_dist_survival(params_g, N, C,
                                                                   ...
                                                                   succ_delta_t_dict,
                                                                   n_fail_edges, fail_mask_idx, fail_scatter_idx, fail_delta_t_dict,
                                                                    delta_spatial_dist, include_spatial_dist, include_epsilon,
                                                                   include_genome_dist, succ_delta_g_dict,
                                                                   include_unactivated_nodes, beta, gamma)
...
def construct_digraph_from_inferred_edgelist_single_layer_phase(graph_single_layer_filepath, df_inferred_edgelist, df_map_data=None):
    ...
    for index, row in df_inferred_edgelist.iterrows():
        ...
        if not graph.has_node(src_geonameId):
            graph.add_node(src_geonameId, size=40, lat=src_lat, lng=src_lon, color="blue")
        if not graph.has_node(trgt_geonameId):
            graph.add_node(trgt_geonameId, size=40, lat=trgt_lat, lng=trgt_lon, color="blue")
...
def perform_MultiC_single_layer_phase(preprocessed_events_filepath, date_start, date_end, cascades_info_filepath, \
    ...
    genome_dist_df = pd.read_csv(genome_dist_matrix_filepath, sep=";", keep_default_na=False, index_col=0, header=0)
    ...
    succ_edges_list, succ_mask_idx, succ_scatter_idx, succ_delta_g_dict, succ_delta_t_dict, fail_edges_list, fail_mask_idx, fail_scatter_idx, fail_delta_t_dict, delta_spatial_dist, succ_node_idx = perform_data_parsing_and_tensor_construction_single_layer_phase(\
                    df_events, date_start, date_end, cascade_list, spatial_dist_matrix, genome_dist_df, map_info, out_dist_values_filepath)
    n_succ_edges = len(succ_edges_list)
